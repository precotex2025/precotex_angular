...
@Component({
  selector: 'app-saldo-devolver',
  templateUrl: './saldo-devolver.component.html',
  styleUrls: ['./saldo-devolver.component.scss']
})
export class SaldoDevolverComponent implements OnInit {
...
generateExcel() {
  ...
  let reportData = {
    title: 'REPORTE SALDOS POR DEVOLVER',
...
MostrarCabeceraPaquetes() {
  ...
  this.seguridadControlVehiculoService.mantenimientoCorteSaldos(this.Fec_Inicio, this.Fec_Fin).subscribe(
    (result: any) => {
      if (result.length > 0) {
