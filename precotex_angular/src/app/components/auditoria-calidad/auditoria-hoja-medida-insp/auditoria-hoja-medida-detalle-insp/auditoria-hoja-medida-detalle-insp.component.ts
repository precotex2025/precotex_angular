...
import { DialogRegistroHojaMedidaInspComponent } from '../dialog-registro-hoja-medida-insp/dialog-registro-hoja-medida-insp.component';
...
@Component({
  selector: 'app-auditoria-hoja-medida-detalle-insp',
  templateUrl: './auditoria-hoja-medida-detalle-insp.component.html',
  styleUrls: ['./auditoria-hoja-medida-detalle-insp.component.scss']
})
export class AuditoriaHojaMedidaDetalleInspComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
    ...
    this.formulario = formBuilder.group({
      ...
      inputVersion: [''],
      inputOps: [''],
      Linea: ['101', Validators.required],
      CodSupervisor: ['E-2889', Validators.required],
      Supervisor: [''],
  ...
  ngOnInit(): void {
    ...
    if (GlobalVariable.Cod_Hoja_Medida_Cab == 0 && GlobalVariable.vCod_Rol != 6) {
      ...
      this.formulario.controls['OP2'].disable()
      this.formulario.controls['inputCliente'].disable()
      this.formulario.controls['inputEstCli'].disable()
      this.formulario.controls['inputEstPro'].disable()
      ...
      this.formulario.controls['Lin_Pro'].disable()
      this.formulario.controls['Temporada'].disable()
  ...
  getHojasMedida(){
    this.auditoriaHojaMedidaService.ObtenerAuditoriaHojaMedida(
      this.Cod_EstPro,
      this.Cod_Version,
      'A'
    ).subscribe(
      (result: any) => {
        ...
        if (result.length > 0) {
          this.dataHojasMedida = result;
        } else {
          this.matSnackBar.open('No se encontraron hojas de medida operativas.', 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 1500 })
        }
      },
  ...
  verificarEstadoFicha() {
    ...
    this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaCabeceraInsp(
      this.Cod_Accion,
      this.Cod_Hoja_Medida_Cab,
      this.Cod_OrdPro,
      this.Cod_ColCli,
      ...
      this.Fecha_Auditoria,
      this.Fecha_Auditoria2,
      this.formulario.get('Tipo_Prenda').value,
      this.formulario.get('Temporada').value,
      this.formulario.get('Lin_Pro').value,
      this.formulario.get('OP2').value,
      this.formulario.get('inputEstCli2').value,
      ''
    ).subscribe(
      (result: any) => {
    ...
    this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaCabeceraInsp(
      ...
      (result: any) => {
        if (result.length > 0) {
          ...
          this.formulario.controls['OP2'].setValue(result[0].Cod_OrdPro2)	
          this.formulario.controls['inputEstCli2'].setValue(result[0].Cod_EstCli2)	
          this.formulario.controls['Sec'].setValue(result[0].Sec != null ? result[0].Sec.toString() : 0)
      ...
      this.auditoriaHojaMedidaService.AuditoriaHojaMedidaPrendaService(
        ...
        (result: any) => {
          ...
          if (result.length > 0) {
            ...
            this.Cod_EstCli = result[0].Cod_EstCli
            this.Cod_TemCli = result[0].Cod_TemCli
            this.formulario.controls['Tipo_Prenda'].setValue(result[0]['Tipo de Prenda'])
  ...
  buscarPorOp2() {
    this.Cod_OrdPro2 = this.formulario.get('OP2')?.value
    if (this.Cod_OrdPro2.length == 5) {
      this.Cod_Accion = '4'
      this.Cod_EstCli = ''
      this.auditoriaHojaMedidaService.AuditoriaHojaMedidaPrendaService(
        this.Cod_Accion,
        ...
        (result: any) => {
          ...
          if (result.length > 0) {
            this.Cod_EstCli2 = result[0].Cod_EstCli
            this.formulario.controls['inputEstCli2'].setValue(result[0].Cod_EstCli)
          } else {
            this.matSnackBar.open('No hay datos con la OP ingresada...', 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 1500 })
  ...
  buscarTemporada() {
    this.auditoriaHojaMedidaService. Cf_Busca_Derivado_TemporadaCliente(
      this.Cod_Cliente,
      this.Cod_EstCli
    ).subscribe(
      (result: any) => {
        ...
        if (result.length > 0) {
          var temporada = result.filter(element => {
            return element.Codigo === this.Cod_TemCli
          });

          console.log(temporada);
          this.formulario.patchValue({
            Temporada: temporada[0].Descripcion
          });
  ...
  obtenerLinea() {
    this.auditoriaHojaMedidaService.CF_ObtenerLinea(
      this.Cod_OrdPro
    ).subscribe(
      (result: any) => {
        console.log(result[0].Linea)
        if (result.length > 0) {
          

          
          this.formulario.patchValue({
            Lin_Pro: result[0].Linea
          });
  ...
  traerRegistrosDetalle(Cod_Hoja_Medida_Cab: number, Sec: number, Tip_Medida: string, Sec_Medida: string, Cod_Talla: string) {
    ...
    this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaDetalleInsp(
      this.Cod_Accion,
      this.Cod_Hoja_Medida_Det,
      this.Cod_Hoja_Medida_Cab,
      this.Sec,
  ...
  onCellClicked(e: CellClickedEvent, params): void {
    ...
    if (this.Flg_Enable == true) {
      if (ultimoCaracter == '1' || ultimoCaracter == '2' || ultimoCaracter == '3' || ultimoCaracter == '4' || ultimoCaracter == '5') {
        let dialogRef = this.dialog.open(DialogRegistroHojaMedidaInspComponent,
          {
            disableClose: true,
            panelClass: 'my-class',
            data: {
              Cod_Talla: e.colDef.field,
  ...
  MostrarCargaMedida() {
    ...
    this.Cod_Hoja_Medida_Cab
    this.auditoriaHojaMedidaService.AuditoriaHojaMedidaCargaMedidaServiceInsp(
      this.Cod_EstPro,
      this.Cod_Version,
      ...
      if (result == 'true') {
        ...
        this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaCabeceraInsp(
          ...
          (result: any) => {
            ...
            if (result[0].Respuesta == 'OK') {
              this.router.navigate(['/AuditoriaMedida']);
