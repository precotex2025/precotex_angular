...
@Component({
  selector: 'app-dialog-registrar-salida-acabado',
  templateUrl: './dialog-registrar-salida-acabado.component.html',
  styleUrls: ['./dialog-registrar-salida-acabado.component.scss']
})
export class DialogRegistrarSalidaAcabadoComponent implements OnInit {
  ...
  //dataModificadaCheck: Array<data_op> = [];
  //dataModificada: Array<data_op> = [];
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<DialogRegistrarSalidaAcabadoComponent>
  ) {
  ...
  ngOnInit(): void {
    ...
    if(this.data.Num_Auditoria!=0){
      ...
      this.formulario.controls['Cod_TemCli'].setValue(this.data.Cod_TemCli)
    }    
      ...
      .subscribe((res) => {
        ...
        if(res[0].Respuesta == "OK"){
          this.dataCheck.forEach(element => {
            ...
            this.auditoriaAcabadosService.Mant_AuditoriaModuloSalidaAcabadoOp(
              lc_accion,
  ...
  selectTemporada(item: any){
    ...
    console.log(item)
    this.auditoriaAcabadosService.Mant_AuditoriaModuloSalidaAcabadoOp('L', 0, numAuditoria, '', item.Cod_EstCli.trim(), item.Cod_TemCli.trim(), '', '')
      .subscribe((res) => {
        this.dataOrdPro = res;
