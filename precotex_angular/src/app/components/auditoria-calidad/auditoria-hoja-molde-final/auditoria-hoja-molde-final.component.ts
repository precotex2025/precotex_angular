...
import { DialogAuditoriaHojaFinalRegistroComponent } from './dialog-auditoria-hoja-final-registro/dialog-auditoria-hoja-final-registro.component';
...
@Component({
  selector: 'app-auditoria-hoja-molde-final',
  templateUrl: './auditoria-hoja-molde-final.component.html',
  styleUrls: ['./auditoria-hoja-molde-final.component.scss']
})
export class AuditoriaHojaMoldeFinalComponent implements OnInit {
   ...
   range = new FormGroup({
    ...
    displayedColumns: string[] = ['Id_Hoja_Medida', 'Cod_OrdPro', 'Nom_TemCli','Cod_ColCli', 'Nom_Cliente', 'Nom_Auditor', 'Fecha_Registro', 'Flg_Estado','Acciones']
  ...
  listaraAuditriaHojaMedidaMoldes(){
    ...
    //this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes('L', 0, this.formulario.get('OP')?.value.trim(), '', '0' , '', '', this.datepipe.transform(this.range.get('start')?.value, 'yyyy-MM-ddTHH:mm:ss'), this.datepipe.transform(this.range.get('end')?.value, 'yyyy-MM-ddTHH:mm:ss'))
  ...
  onAgregarRegistro(){
    ...
    let dialogRef = this.dialog.open(DialogAuditoriaHojaFinalRegistroComponent, {
      disableClose: true,
  ...
  onGenerarExcel(data_det: data_det){
    ...
    this.auditoriaHojaMedidaService.AuditoriaHojaMedidaMoldesFinalTallas(data_det.Cod_EstPro, data_det.Cod_Version, data_det.Cod_OrdPro, data_det.Sec, data_det.Id_Hoja_Medida)
      .subscribe((result: any) => {
        ...
        let reportData = {
          title: 'REPORTE REGISTRO HOJA DE MEDIDA FINAL - MOLDES Nº DE FICHA ' + data_det.Id_Hoja_Medida.toString(),
  ...
  onEliminarRegistro(Id_Hoja_Medida: number){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result == 'true') {
        ...
        //this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes('D', Id_Hoja_Medida, '', '', '', '', '', '', '')
        this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes(formData)
        .subscribe((result: any) => {
