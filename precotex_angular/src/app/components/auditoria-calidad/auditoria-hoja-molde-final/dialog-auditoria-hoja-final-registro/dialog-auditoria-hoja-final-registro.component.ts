...
import { DialogAuditoriaHojaFinalMedidasComponent } from '../dialog-auditoria-hoja-final-medidas/dialog-auditoria-hoja-final-medidas.component';
...
@Component({
  selector: 'app-dialog-auditoria-hoja-final-registro',
  templateUrl: './dialog-auditoria-hoja-final-registro.component.html',
  styleUrls: ['./dialog-auditoria-hoja-final-registro.component.scss']
})
export class DialogAuditoriaHojaFinalRegistroComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogAuditoriaHojaFinalRegistroComponent>,
  ...
  onConformarRegistro(formDirective): void{
    ...
    /*this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes(accion, 
      this.data.Id_Hoja_Medida, 
  ...
  onFinalizarFicha(){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result == 'true') {
        ...
        //this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes('F', this.data.Id_Hoja_Medida, '', '', '', '', '2', '', '')
        this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes(formData)
          .subscribe((result: any) => {        
            if (result[0].Respuesta == 'OK') {
  ...
  onBuscarOP(){
    ...
    if (codOrdPro.length == 5) {
      //this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes('O', 0, codOrdPro, '', '0' , '', '', '', '')
      this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes(formData)
        .subscribe((result: any) => {
          //console.log(result)
          if (result.length > 0) {
  ...
  cargaDetalleHojaMedida() {
    ...
    this.auditoriaHojaMedidaService.AuditoriaHojaMedidaMoldesFinalTallas(
  ...
  onCellClicked(e: CellClickedEvent, params): void{
    ...
    if (this.flgEnable == true) {
      if (ultimoCaracter == '1' || ultimoCaracter == '2' || ultimoCaracter == '3' || ultimoCaracter == '4' || ultimoCaracter == '5') {
        ...
        let dialogRef = this.dialog.open(DialogAuditoriaHojaFinalMedidasComponent, {
          disableClose: true,
          panelClass: 'my-class',
          data: data_det
        });
  ...
  onCargarColorOP(codOrdPro: string){
    ...
    //this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes('C', 0, codOrdPro, '', '0' , '', '', '', '')
    this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes(formData)
      .subscribe((result: any) => {
        console.log(result)
        if (result.length > 0) {
  ...
  onCargarMedidaOP(codOrdPro: string){
    ...
    //this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes('H', 0, codOrdPro, '', '0' , '', '', '', '')
    this.auditoriaHojaMedidaService.MantenimientoAuditoriaHojaMedidaFinalMoldes(formData)
      .subscribe((result: any) => {
        //console.log(result)
        if (result.length > 0) {
