...
@Component({
  selector: 'app-recursos-programacion-vacaciones',
  templateUrl: './recursos-programacion-vacaciones.component.html',
  styleUrls: ['./recursos-programacion-vacaciones.component.scss']
})
export class RecursosProgramacionVacacionesComponent implements OnInit {
  ...
  selectEmpresa(event) {
    ...
    if (event.value != '') {
      ...
      this.dias = 0;
      this.despachoTelaCrudaService.verUsuariosBd(event.value).subscribe(res => {
        if (res != null) {
          this.dataTrabajadores = res;
        } else {
  ...
  selectUsuario(event) {
    ...
    if (event != '' && event != undefined) {
      this.data = [];
      this.Cod_Trabajador = event.Cod_Trabajador;
      this.Tip_Trabajador = event.Tip_Trabajador;
    } else {
      this.Cod_Trabajador = '';
      this.Tip_Trabajador = '';
      this.fecha_inicio = '';
              ...
              if (this.data[0].dias_10 > 0) {
                ...
                if (dias > 0) {
                  ...
                  if (truncos < dias) {
                    ...
                    console.log('truncos2')
                    var guardar = dias;
                    dias = truncos - dias;
                    var periodo_10 = this.data[0].Periodo_10;
                    if(this.data[0].dias_truncos != '' && this.data[0].dias_vencidos == null && this.data[0].Periodo_10 == this.Fec_Ingreso){
                      periodo_10 = this.data[0].Periodo_10;
                    }else{
                      periodo_10 = Number(this.data[0].Periodo_10) + 1;
                    }
            ...
            this.dataPeriodos.forEach(element => {
              ...
              if (element == this.data[0].Periodo_10) {
                if (dias > 0) {
                  if (this.data[0].dias_10 < dias) {
                    console.log('10.1')
                    console.log(this.data[0].dias_10);
                    dias = dias - this.data[0].dias_10;
                    if (fecha_fin == '') {
                      var res = new Date(this.fecha_inicio);
                    ...
                    if (fecha_fin == '') {
                      var res = new Date(this.fecha_inicio);
                      var fec_inicio = new Date(this.fecha_inicio);
                      ...
                      let fecha_inicio_modificada = _moment(fec_inicio.valueOf()).format('DD/MM/YYYY');
                      console.log(fecha_inicio_modificada)
                      var data2 = {
                        periodo: element,
                        dias: guardar,
                        Cod_Trabajador: this.Cod_Trabajador,
          ...
          //SI LOS DÍAS DE VACACIONES ABARCAN TRUNCOS
            ...
            this.dataPeriodos.forEach(element => {
              ...
              if (element == this.data[0].Periodo_10) {
                if (this.data[0].dias_10 > 0) {
                  if (dias > 0) {
                    if ((this.data[0].dias_09 != null) && this.data[0].dias_09 < dias) {
                      console.log('11.4')
                      dias = dias - this.data[0].dias_09;
                      if (fecha_fin == '') {
                        var res = new Date(this.fecha_inicio);
                        ...
                        if(dias > 0){
                          ...
                          let fecha_modificada = _moment(res.valueOf()).format('DD/MM/YYYY');
                          res.setDate(res.getDate() + (dias - 1));
                          fecha_inicio = _moment(res.valueOf()).format('DD/MM/YYYY');
                          var datos = {
                            periodo: Number(element) + 1,
                            dias: dias,
                            Cod_Trabajador: this.Cod_Trabajador,
                            Cod_Empresa: this.Cod_Empresa,
                            Tip_Trabajador: this.Tip_Trabajador,
                            fecha_inicio: fecha_modificada,
                            fecha_fin: fecha_inicio
                          };
                          console.log(datos);
                          dataPeriodos.push(datos)
                        }
                      }

                    } else if ((this.data[0].dias_10 != null) && this.data[0].dias_10 < dias) {
                      console.log('11.41')
                      dias = dias - this.data[0].dias_10;
                      if (fecha_fin == '') {
                        var res = new Date(this.fecha_inicio);
                        ...
                        if (dias > 0) {
                          ...
                          console.log(dias)
                          dias = dias;
                          var res2 = new Date(this.fecha_inicio);
                          res2.setDate(res2.getDate() + this.data[0].dias_10 + 1);
                          let fecha_modificada2 = _moment(res2.valueOf()).format('DD/MM/YYYY');
                          fecha_fin = _moment(res2.valueOf()).format('YYYY-MM-DD');
                          res2.setDate(res2.getDate() + (dias - 1));
                          let fecha_inicio_modificada2 = _moment(res2.valueOf()).format('DD/MM/YYYY');
                          let data = {
                            periodo: parseInt(element) + 1,
                            dias: dias,
                            Cod_Trabajador: this.Cod_Trabajador,
                            Cod_Empresa: this.Cod_Empresa,
                            Tip_Trabajador: this.Tip_Trabajador,
                            fecha_inicio: fecha_modificada2,
                            fecha_fin: fecha_inicio_modificada2
                          };
                          dataPeriodos.push(data)
                        }
    ...
    if (this.Cod_Trabajador != '' && this.Cod_Empresa != '') {
      ...
      this.despachoTelaCrudaService.getControlVacacionesTrabajador(this.Cod_Trabajador, this.Tip_Trabajador, this.Cod_Empresa).subscribe((result: any) => {
        ...
        if (result != null) {
          ...
          this.Fec_Ingreso = this.data[0].Fec_Ingreso.date.substring(0, 4);
          console.log(this.Fec_Ingreso);
