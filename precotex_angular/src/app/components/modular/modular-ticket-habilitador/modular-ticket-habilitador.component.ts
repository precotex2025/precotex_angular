...
import { DialogConfirmacionEstampadoComponent } from 'src/app/components/dialogs/dialog-confirmacion-estampado/dialog-confirmacion-estampado.component';
//import { DialogParticionTicketHabilitadorComponent } from './dialog-particion-ticket-habilitador/dialog-particion-ticket-habilitador.component';
...
@Component({
  selector: 'app-modular-ticket-habilitador',
  templateUrl: './modular-ticket-habilitador.component.html',
  styleUrls: ['./modular-ticket-habilitador.component.scss']
})
export class ModularTicketHabilitadorComponent implements OnInit {
  ...
  listadoTicket() {
    ...
    this.inspeccionPrendaService.CF_MUESTRA_LISTADO_GENERAR_TICKET_WEB(
  ...
  OpenDialogConfirmacion(item: any, usu: string, modulo: string, compostura: string, estampado: string, zurcido: string, desmanche: string, retoque: string) {
    let dialogRef = this.dialog.open(DialogConfirmacionEstampadoComponent, {
      disableClose: true,
      data: {
        COD_ORDPRO: item.COD_ORDPRO,
        COD_TALLA: item.COD_TALLA
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result == 'true') {
        ...
        this.inspeccionPrendaService.CF_MUESTRA_GENERACION_TICKET_FAMILIA_WEB(
          ...
          (result: any) => {
            ...
            if (result[0].Respuesta == 'OK') {
              setTimeout(() => {
                this.SpinnerService.hide();
                this.getOrganico(item, result[0].Num_Paquete, result[0].Ticket)
                this.listadoTicket();
              }, 1000);
            } else {
        ...
        //Envio externo, genera movimiento
        this.inspeccionPrendaService.CF_MUESTRA_GENERACION_TICKET_FAMILIA_WEB(
          '001',
          item.COD_ORDPRO,
          item.COD_PRESENT,
  ...
  /*OpenDialogParticion(items){

    let dialogRef = this.dialog.open(DialogParticionTicketHabilitadorComponent, {
      disableClose: true,
      data: items
    });
  ...
  touchTicket(item) {
    ...
    //console.log(ruta_prenda)
    console.log(
      '001',
      'COD_ORDPRO: ',item.COD_ORDPRO,
      'COD_PRESENT: ',item.COD_PRESENT,
      'COD_TALLA: ',item.COD_TALLA,
      'CANTIDAD: ',item.CANTIDAD,
      'COD_ORDPRO: ',GlobalVariable.vusu,
      'modulo: ',this.formulario.get('Modulo').value,
      'Ruta_Inspeccion: ',item.Ruta_Inspeccion,
    ...
    let condicionRuta = item.Ruta_Inspeccion.trim().substr(0, 1)
    if(condicionRuta == 'E'){
      let data = {
        "CANTIDAD": item.CANTIDAD,
        ...
        "COD_PRESENT": item.COD_PRESENT,
        "COD_TALLA": item.COD_TALLA,
        "DES_PRESENT": item.DES_PRESENT,
        "FEC_CREACION": item.FEC_CREACION,
        "ICONO_WEB": item.ICONO_WEB,
        "Ruta_Inspeccion": item.Ruta_Inspeccion,
        "SEL": item.SEL,
        "STOCK": item.STOCK,
        "Tipo_Ticket": item.Tipo_Ticket,
        ...
        "desmanche": desmanche,
        "retoque": retoque,
        "Des_Modulo": this.Des_Modulo,
        "Familia": this.formulario.get('Familia').value
      }
      this.OpenDialogConfirmacion(item, GlobalVariable.vusu, this.formulario.get('Modulo').value, compostura, estampado, zurcido, desmanche, retoque)
      //this.OpenDialogParticion(data)
    }else{
  ...
  movimientoTicket(item, Num_Paquete, Ticket) {
    this.SpinnerService.show();
    this.inspeccionPrendaService.CF_MODULAR_MOVI_INSPECCION_AUTOMATICO_V2(
      this.formulario.get('Modulo').value,
      this.formulario.get('Familia').value,
      GlobalVariable.vusu,
      Ticket,
