...
interface data_det {
  ...
  Cod_Auditor: string;
  Auditor: string;
  Cod_AudTransito: string;
  Auditor_Transito: string;
  Usuario_Transito: string;
}
...
@Component({
  selector: 'app-modular-reporte-auditoria-salida-inspeccion',
  templateUrl: './modular-reporte-auditoria-salida-inspeccion.component.html',
  styleUrls: ['./modular-reporte-auditoria-salida-inspeccion.component.scss']
})
export class ModularReportesAuditoriaSalidaInspeccionComponent implements OnInit {
  ...
  generateExcel() {
    ...
    if (this.dataSource.data.length == 0) {
      ...
      let reportData = {
        title: 'REPORTE DE AUDITORIA SALIDA EN INSPECCION',
  ...
  buscarReporteModularAuditoriaSalida() {
    ...
    this.SeguridadActivoFijoReporteService.CF_Modular_Reporte_Auditoria_Salida(
      accion,
      Cod_Modulo,
      this.fIni,
      this.fFin
