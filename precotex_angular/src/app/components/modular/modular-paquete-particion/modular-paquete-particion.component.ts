...
@Component({
  selector: 'app-modular-paquete-particion',
  templateUrl: './modular-paquete-particion.component.html',
  styleUrls: ['./modular-paquete-particion.component.scss']
})
export class ModularPaqueteParticionComponent implements OnInit {
  ...
  listadoTicket() {
    ...
    this.inspeccionPrendaService.CF_MODULAR_LISTADO_GENERAR_TICKET_PARTICION_RETOQUE_WEB(
      'L',
      this.formulario.get('Modulo').value,
      this.formulario.get('sNumTicket').value
    ).subscribe(
      (result: any) => {
        console.log(result);
        if(result.length > 0){
          ...
          this.matSnackBar.open('No se encontraron registros en el módulo seleccionado.', 'Cerrar', {
            duration: 5000,
            verticalPosition:'top'
          });
          this.dataPrendasImp = [];
    ...
    this.inspeccionPrendaService.CF_MODULAR_LISTADO_GENERAR_TICKET_PARTICION_RETOQUE_WEB(
      ...
      (result: any) => {
        ...
        if(result.length > 0){
          ...
          result.forEach((obj : any, i :any) => {
            let info = {
              'Ticket': obj.Ticket,
              'CANTIDAD': obj.Cantidad,
              'Num_MovStk': obj.Num_MovStk,
              ...
              'Num_Paquete': item.Num_Paquete              
            }
            console.log(info)
            if(i == 0){
              this.getOrganico(info);
            }else{
              setTimeout(() => {
                this.getOrganico(info);
              }, 2000);
  ...
  getOrganico(item){
    ...
    //HABILITAR EN AMBIENTE DE PRODUCCION
              ...
              <div class="flex" style="flex-direction:column;">
                  <div class="print caja" style="width:100%;font-size:9px; margin-left:5px;">
                    ...
                    <div style="display:flex;">
                      <div style="display:flex; justify-content:center; margin-left:5px;">
                        <span style="text-align:center">Mov: ${item.Tipo_Movimiento}- ${item.Num_MovStk}</span> 
                      </div>          
  ...
  touchParticion(item){
    let data = {
      "Ticket_Habilitador": item.Ticket,
      "Id": item.Id,
      ...
      "Estampado": item.Estampado_Bordado,
      "Zurcido": item.Zurcido,
      "Desmanche": item.Desmanche,
      "Retoque": item.Retoque,
