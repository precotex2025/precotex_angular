...
import { DialigDefectosAuditoriaComponent } from './dialig-defectos-auditoria/dialig-defectos-auditoria.component';
import { InspeccionPrendaService } from 'src/app/services/inspeccion-prenda.service';

@Component({
  selector: 'app-modular-auditoria-modulo',
  templateUrl: './modular-auditoria-modulo.component.html',
  styleUrls: ['./modular-auditoria-modulo.component.scss']
})
export class ModularAuditoriaModuloComponent implements OnInit {
  ...
  agregarTicket() {
    ...
    if (ticket.length >= 11) {
      ...
      this.auditoriaService.CF_Modular_Auditoria_Muestra_Paquete('M', ticket).subscribe(
        (res: any) => {
          this.SpinnerService.hide();

          if (res[0].status == 1) {
            if (this.dataPaquetes.length == 0) {
              this.dataPaquetes.push(res[0]);
              this.formulario.patchValue({
                Ticket: ''
              });
            } else {
              var array = this.dataPaquetes.filter(element => {
                return element.Cod_OrdPro == res[0].Cod_OrdPro && element.Cod_Present == res[0].Cod_Present && res[0].Tipo_Ticket == element.Tipo_Ticket;
              });
              ...
              if (array.length > 0) {
                ...
                this.matSnackBar.open('SOLO PUEDES AGRUPAR PAQUETES DE LA MISMA OP, COLOR Y TIPO DE PROCESO ', 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 3000 })
              }
            }
