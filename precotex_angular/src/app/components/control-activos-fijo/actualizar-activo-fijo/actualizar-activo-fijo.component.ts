...
@Component({
  selector: 'app-actualizar-activo-fijo',
  templateUrl: './actualizar-activo-fijo.component.html',
  styleUrls: ['./actualizar-activo-fijo.component.scss']
})
export class ActualizarActivoFijoComponent implements OnInit {
    ...
    this.route.queryParams.subscribe((res) => {
      ...
      if (res != null) {
        this.formulario.patchValue({
          ...
          Cod_Activo_Fijo: res['Cod_Activo_Fijo'],
          Empresa: Number(res['Cod_Empresa']),
          Sede:  res['Cod_Establecimiento'],
          Piso: res['Num_Piso'],
          Ccosto: res['Cod_CenCost'],
          Area: res['Nom_Area'],
          Responsable: res['Nom_Responsable'],
          Usuario: res['Nom_Usuario'],
          CodAct: res['Cod_Activo'],
          ClaseAct: Number(res['Cod_Categoria']),
          Ubicacion: res['Ubicacion'],
          Descripcion: res['Descripcion'],
          Marca: res['Nom_Marca'],
          Modelo: res['Nom_Modelo'],
          Serie: res['Num_Serie_Equipo'],
          Estado: Number(res['Estado_Fisico']),
          Uso: Number(res['Uso_Desuso']),
          Observacion: res['Observacion'],
          Color: res['Color'],
          Medidas: res['Medida'] != '' ? res['Medida'].trim() : '',
          Motor: res['Num_Serie_Motor'] ,
          Chasis: res['Num_Serie_Chasis'],
          Placa: res['Num_Placa'],
          Combustible: res['Tipo_Combustible'],
          Caja: res['Tipo_Caja'],
          Asiento: res['Cant_Asiento'],
          Ejes: res['Cant_Eje'],
        });

        this.selectEmpresa(res['Cod_Empresa']);
        this.CambiarContenidoDetalle(res['Cod_Categoria']);
      }
    })
  }
      ...
      this.controlActivoFijoService.MantenimientoActivoFijoService(
        ...
        (result: any) => {
          ...
          if(result[0].Respuesta == 'OK'){
            ...
            let data = {
              start: this.formulario.get('start').value,
              end: this.formulario.get('end').value
            }
            this._router.navigate(['/HistorialActivosFijos'], {queryParams: data, skipLocationChange:true});
