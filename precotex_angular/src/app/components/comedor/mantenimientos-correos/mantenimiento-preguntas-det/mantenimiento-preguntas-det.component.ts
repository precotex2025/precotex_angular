...
@Component({
  selector: 'app-mantenimiento-preguntas-det',
  templateUrl: './mantenimiento-preguntas-det.component.html',
  styleUrls: ['./mantenimiento-preguntas-det.component.scss']
})
export class MantenimientoPreguntasDetComponent implements OnInit {
  ...
  OpenDialogConfirmacion(data) {
    let dialogRef = this.dialog.open(CrearPreguntasComedorComponent, {
      disableClose: false,
      //minWidth: '600px',
      panelClass: 'my-class',
      maxWidth: '95%',
  ...
  OpenDeleteConfirmacion(data) {
    ...
    if (confirm('Esta seguro(a) de eliminar el siguiente registro?')) {
      ...
      this.encuestasService.obtenerPreguntasComedor(
        'D',
        '',
        data.Id_Detalle,
        ''
  ...
  openDialogDetalle() {
    this._router.navigate(['/MantenimientoCorreos']);
  }
