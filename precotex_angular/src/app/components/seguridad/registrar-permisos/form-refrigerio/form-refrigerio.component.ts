...
@Component({
  selector: 'app-form-refrigerio',
  templateUrl: './form-refrigerio.component.html',
  styleUrls: ['./form-refrigerio.component.scss']
})
export class FormRefrigerioComponent implements OnInit {
  ...
  formulario = this.formBuilder.group({ start: [new Date], end: [new Date], jefe: [this.sCod_Usuario], tipos: ['004'], motivos: [''], observaciones: [''], Sede_partida: [''] })
  ...
  opendialog() {
    ...
    let dialogRef = this.dialog.open(DialogIngresoEmpleadorefrigerioComponent, {
      disableClose: true,
      minHeight: '400px',
      panelClass: 'my-class',
      maxWidth: '95vw',
  ...
  guardarPermisos() {
    ...
    if (this.formulario.get('tipos').value != '' && this.formulario.get('Sede_partida').value != '') {
      ...
      this.boton = false;
      this.matSnackBar.open('Debes seleccionar la sede de partida', 'Cerrar', {
        duration: 1500,
      })
    }
