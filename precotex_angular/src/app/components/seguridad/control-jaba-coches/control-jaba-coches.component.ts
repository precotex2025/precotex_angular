...
import { DialogDetalleIngresoPackComponent } from './dialog-detalle-ingreso-pack/dialog-detalle-ingreso-pack.component';




@Component({
  selector: 'app-control-jaba-coches',
  templateUrl: './control-jaba-coches.component.html',
  styleUrls: ['./control-jaba-coches.component.scss']
})
export class ControlJabaCochesComponent implements OnInit {
  ...
  detalleJaba(Cod_Packing_Jaba){
    ...
    let dialogRef = this.dialog.open(DialogDetalleIngresoPackComponent, {
      data: {
  ...
  changeJaba() {
    ...
    this.seguridadControlGuiaService.Lg_Man_Packing_Jaba_Web('D', 0, GlobalVariable.vusu, GlobalVariable.num_planta, GlobalVariable.num_planta, GlobalVariable.vusu, this.Num_Jaba, '').subscribe(
      (result: any) => {
        if (result.length > 0) {
