...
@Component({
  selector: 'app-almacen-telas-corte',
  templateUrl: './almacen-telas-corte.component.html',
  styleUrls: ['./almacen-telas-corte.component.scss']
})
export class AlmacenTelasCorteComponent implements OnInit {
  Fecha_Auditoria = ""
  Fecha_Auditoria2 = ""
  ...
  changeTela(event, Num_Solicitud){
    console.log(event)
    this.reposicionesService.insertarFechasReposicion('T', Num_Solicitud, event.value).subscribe(
      (result: any) => {
        console.log(result);
        if(result[0].status == 1){
          this.matSnackBar.open('Se registro el tipo de tela correctamente.', 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 2500 });
        } else {
          this.matSnackBar.open(result[0].Respuesta, 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 2500 });
          this.obtenerReposiciones();
        }
