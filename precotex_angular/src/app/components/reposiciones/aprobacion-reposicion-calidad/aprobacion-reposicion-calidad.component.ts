...
@Component({
  selector: 'app-aprobacion-reposicion-calidad',
  templateUrl: './aprobacion-reposicion-calidad.component.html',
  styleUrls: ['./aprobacion-reposicion-calidad.component.scss']
})
export class AprobacionReposicionCalidadComponent implements OnInit {
  ...
  changeAprobacion(event, Num_Solicitud) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
      if (result != '') {
        this.reposicionesService.saveAprobReposiciones('C', Num_Solicitud, result['observaciones'], result.estado).subscribe(
          (res: any) => {
            console.log(res);
