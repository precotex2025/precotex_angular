...
@Component({
  selector: 'app-aprobacion-reposicion-corte',
  templateUrl: './aprobacion-reposicion-corte.component.html',
  styleUrls: ['./aprobacion-reposicion-corte.component.scss']
})
export class AprobacionReposicionCorteComponent implements OnInit {
  Fecha_Auditoria = ""
  Fecha_Auditoria2 = ""
  ...
  obtenerReposiciones() {
    ...
    this.reposicionesService.getReposiciones('L', '', this.range.get('start').value, this.range.get('end').value, this.Estado, 'T').subscribe(
      (result: any) => {
        console.log(result);
        if (result != false) {
  ...
  changeCheck(event, Num_Solicitud){
    ...
    if(event.checked == true){
      this.reposicionesService.insertarFechasReposicion('TT', Num_Solicitud).subscribe(
        (result: any) => {
          console.log(result);
          this.obtenerReposiciones();
          if (result[0].status == 1) {
            this.matSnackBar.open('Se registro la fecha de ingreso correctamente.', 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 2500 });
          } else {
  ...
  changeCheck2(event, Num_Solicitud){
    console.log(event);
    if(event.checked == true){
      this.reposicionesService.insertarFechasReposicion('TS', Num_Solicitud).subscribe(
        (result: any) => {
          console.log(result);
          this.obtenerReposiciones();
          if (result[0].status == 1) {
            this.matSnackBar.open('Se registro la fecha de salida correctamente.', 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 2500 });
          } else {
            this.matSnackBar.open(result[0].Respuesta, 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 2500 });
          }
        },
  ...
  changeAprobacion(event, Num_Solicitud){
    console.log(event);
    ...
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
      if(result != ''){
        this.reposicionesService.saveAprobReposiciones('T', Num_Solicitud, result['observaciones'], result.estado).subscribe(
          (res: any) => {
            console.log(res);
