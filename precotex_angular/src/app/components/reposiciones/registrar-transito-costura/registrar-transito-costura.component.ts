...
@Component({
  selector: 'app-registrar-transito-costura',
  templateUrl: './registrar-transito-costura.component.html',
  styleUrls: ['./registrar-transito-costura.component.scss']
})
export class RegistrarTransitoCosturaComponent implements OnInit {
  ...
  displayedColumns: string[] = [
    ...
    'Sede',
    'Num_Linea',
    'Fec_Transito_Costura',
    'Fec_Salida_Transito_Cost',
    'Fec_Recepcion_Transito_Cost',
    'Fec_Recepcion_Salida_Transito',
    'Status_Reposicion',
    'Piezas_Falladas',
    'Piezas_Malogradas',
    'Piezas_Faltantes',
  ...
  changeCheck(event, Num_Solicitud, Accion){
    console.log(event);
    if(event.checked == true){
      this.reposicionesService.insertarFechasReposicion(Accion, Num_Solicitud).subscribe(
        (result: any) => {
          console.log(result);
          this.obtenerReposiciones();
  ...
  insertarReposicion(Num_Solicitud) {
    this.reposicionesService.insertarFechasReposicion('AT', Num_Solicitud ).subscribe(
      (result: any) => {
        console.log(result);
        if(result[0].status == 1){
