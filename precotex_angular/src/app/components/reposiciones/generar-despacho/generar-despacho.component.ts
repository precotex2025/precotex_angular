...
import { DialogGenerarDespachoComponent } from './dialog-generar-despacho/dialog-generar-despacho.component';
...
@Component({
  ...
  styleUrls: ['./generar-despacho.component.scss']
})
export class GenerarDespachoComponent implements OnInit {
  ...
  changeCheck(event, Num_Solicitud){
    ...
    if(event.checked == true){
      this.reposicionesService.insertarFechasReposicion('D', Num_Solicitud).subscribe(
        (result: any) => {
          console.log(result);
          this.obtenerReposiciones();
  ...
  insertarReposicion(Num_Solicitud) {
    this.reposicionesService.insertarFechasReposicion('D', Num_Solicitud ).subscribe(
      (result: any) => {
        console.log(result);
        if(result[0].status == 1){
  ...
  changeAprobacion(event, Num_Solicitud) {
    ...
    let dialogRef = this.dialog.open(DialogGenerarDespachoComponent,
      {
        disableClose: true,
        panelClass: 'my-class',
      });
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
      if (result != '') {
        this.reposicionesService.saveDespachoReposicion(Num_Solicitud, result.observaciones, result.piezas_ML, result.piezas_prod, result.reposicion).subscribe(
          (result: any) => {
            console.log(result);
            this.obtenerReposiciones();
            if (result[0].Respuesta == 'OK') {
              this.matSnackBar.open('Se generó el despacho correctamente.', 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 2500 })
            } else {
              this.matSnackBar.open('Ha ocurrido un error al generar el despacho.', 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 2500 })
            }
          },
          (err: HttpErrorResponse) => {
            this.matSnackBar.open(err.message, 'Cerrar', { horizontalPosition: 'center', verticalPosition: 'top', duration: 2500 })
