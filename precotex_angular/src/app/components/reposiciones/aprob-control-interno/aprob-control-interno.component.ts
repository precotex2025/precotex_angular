...
@Component({
  selector: 'app-aprob-control-interno',
  templateUrl: './aprob-control-interno.component.html',
  styleUrls: ['./aprob-control-interno.component.scss']
})
export class AprobControlInternoComponent implements OnInit {
  ...
  obtenerReposiciones() {
    ...
    this.reposicionesService.getReposiciones('I', '', this.range.get('start').value, this.range.get('end').value, this.Estado, 'I', this.sede).subscribe(
      (result: any) => {
        console.log(result);
        if (result != false) {
  ...
  changeAprobacion(event, Num_Solicitud) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
      if (result != '') {
        this.reposicionesService.saveAprobReposiciones('I', Num_Solicitud, result['observaciones'], result.estado).subscribe(
          (res: any) => {
            console.log(res);
